...
class DatabaseHelper {
  ...
  Future<int> inserContact(Contact contact) async{
    Database db = await database;
    return await db.insert(Contact.tblContact, contact.toMap() );
  }
  Future<int> updatContact(Contact contact) async {
    Database db = await database;
    return db.update(Contact.tblContact, contact.toMap(), where: "${Contact.colId} =?", whereArgs: [contact.id] );
  }
